...
const exportToExcel = async () => {
  try {
    ...
    // 请求 API 获取所有符合条件的数据（请确保后端支持大量数据的查询）
...
const addAPIRules = {
  ...
  // remark 和 is_completed 可选，根据实际业务确定
}
    ...
    render(row) {
      return [
        withDirectives(
          ...
          [[vPermission, 'put/api/v1/total/update']]
        ),
        h(
          NPopconfirm,
          {
...
<template>
  <CommonPage show-footer title="总表数据管理">
    <template #action>
      <div>
        <NButton
          v-permission="'post/api/v1/total/create'"
